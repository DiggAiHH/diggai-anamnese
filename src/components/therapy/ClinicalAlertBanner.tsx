...
export function ClinicalAlertBanner({ patientId }: { patientId: string }) {
    ...
    const active = (alerts || []).filter((a: any) => !a.isDismissed);
    const critical = active.filter((a: any) => a.severity === 'EMERGENCY' || a.severity === 'CRITICAL');

    if (active.length === 0) return null;

    return (
        <div className="space-y-2">
            {critical.map((alert: any) => {
            ...
            {active.length > critical.length && (
                <div className="text-xs text-gray-500 px-2">
                    + {active.length - critical.length} weitere Hinweise ({active.filter((a: any) => a.severity === 'WARNING').length} Warnungen, {active.filter((a: any) => a.severity === 'INFO').length} Info)
                </div>
            )}
        </div>
    );
                        ...
                        <div className={`flex items-start gap-3 px-4 py-3 ${sev.bg}`}>
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                    ...
                                    {alert.isRead && <span className="text-xs text-gray-400">✓ gelesen</span>}
                                    {alert.isDismissed && <span className="text-xs text-gray-400">verworfen</span>}
